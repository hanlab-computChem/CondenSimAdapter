...
from ..forcefield.registry import get_force_field, get_force_field_path
from ._subprocess_commons import _run_command
...
def _build_pdb2gmx_input(
    disable_disulfide: bool, his_type: Optional[int], his_repeat_count: int
) -> Optional[str]:
    """
...
def run_pdb2gmx_for_topology(
    ...
    try:
        _run_command(
            cmd,
            cwd=output_dir,
            input_str=_build_pdb2gmx_input(disable_disulfide, his_type, his_repeat_count),
        )
    except Exception as e:
...
def generate_all_atom_topology(
    ...
    for comp in components:
        ...
        component_topologies.append(
            {"name": comp.name, "topology": str(comp_top_path), "nmol": comp.nmol}
        )
...
def _seq_from_pdb(pdb_path: str) -> str:
    ...
    aa_map = {
        ...
        "HSD": "H",
        "HSE": "H",
        "HSP": "H",  # Histidine protonation states
    ...
    if unknown_residues:
        print(
            f"    Warning: Unknown residues found in PDB (will be marked as 'X'): {unknown_residues}"
        )
