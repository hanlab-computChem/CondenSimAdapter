...
from ..core.config import ComponentType
...
def _get_component_type(comp):
    """Unified component type accessor for new Component and legacy CGComponent."""
    return comp.comp_type if hasattr(comp, "comp_type") else comp.type


def _get_component_pdb_path(comp):
    """Unified pdb_path accessor for new Component and legacy CGComponent."""
    return getattr(comp, "pdb_path", None) or getattr(comp, "fpdb", None)


def _get_component_domains(comp):
    """Unified domain accessor for new Component and legacy CGComponent.

    Returns:
        List of (start, end) tuples, or empty list.
    """
    if hasattr(comp, "folded_domains"):
        return comp.folded_domains  # already List[Tuple[int, int]]
    if hasattr(comp, "fdomains"):
        try:
            return _parse_fdomains(comp.fdomains)
        except (OSError, ValueError, yaml.YAMLError) as e:
            logger.warning(
                "Failed to parse fdomains for component '%s' (file: %s): %s",
                comp.name,
                getattr(comp, "fdomains", "N/A"),
                e,
            )
    return []
...
def generate_plumed_for_minimize(
    ...
    mdp_components = [comp for comp in components if _get_component_type(comp) == ComponentType.MDP]
    ...
    try:
        ...
        if verbose:
            print(f"  Error building global index map for '{topology_dir}': {e}")
        return False
    ...
    for comp in mdp_components:
        domains = _get_component_domains(comp)
        if not domains:
            if verbose:
                print(f"  Warning: No domains found for '{comp.name}', skipping")
            continue

        pdb_path = _get_component_pdb_path(comp)
        if not pdb_path:
            if verbose:
        ...
        for domain_id, (domain_start, domain_end) in enumerate(domains):
            try:
                ...
                if verbose:
                    print(
                        f"    Domain {domain_id}: residues {domain_start}-{domain_end}, {len(pairs)} contact pairs"
                    )
            except (OSError, ValueError, IndexError) as e:
                if verbose:
                    print(
                        f"    Error generating contact map for '{comp.name}' domain "
                        f"{domain_id} ({domain_start}-{domain_end}): {e}"
                    )
    ...
    try:
        ...
        if verbose:
            print(f"  Error writing plumed.dat to '{output_file}': {e}")
        return False
...
def _build_global_index_map(
    topology_dir: Path, components: List[CGComponent]
...
def _generate_contactmap_for_domain(
    ref_structure: str, domain_start: int, domain_end: int, cutoff: float = 4.5
    ...
    pairs, distances = capped_distance(
        heavy_atoms.positions, heavy_atoms.positions, max_cutoff=cutoff, return_distances=True
    )
...
def _write_plumed_dat(contactmaps: List[Dict], output_file: str, kappa: float, verbose: bool):
    """
