...
@click.option("--gpu-id", "-g", type=int, default=0, show_default=True, help="GPU device index.")
@click.option(
    "--solvate", is_flag=True, default=False, help="Add explicit water and ions after minimization."
)
@click.option(
    "--salt-conc",
    type=float,
...
@click.option(
    ...
    default=None,
    help="Box shape for droplet solvation (gmx editconf -bt).",
)
@click.option(
    "--box-distance",
    "-dd",
...
def minimize_command(
    ...
    his_label = {0: "HID (delta, neutral)", 1: "HIE (epsilon, neutral)"}.get(
        his_type_int, str(his_type_int)
    )
    ...
    if disable_disulfide:
        click.echo("  Disulfide:   disabled (-ss)")
