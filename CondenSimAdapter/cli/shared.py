...
def validate_cg_force_field(ctx, param, value):
    """Validate CG force field name, resolving legacy aliases first."""
    if not value:
        return None
    normalized = normalize_cg_ff(value)
    if normalized not in CG_FORCE_FIELDS:
        raise click.BadParameter(
            f"'{value}' is not one of {', '.join(repr(f) for f in CG_FORCE_FIELDS)}."
        )
    return normalized
...
def generate_yaml_with_comments(config: 'CGSimulationConfig', topol: str, 
    ...
    lines = [
        ...
        "",
        "# CG force field",
        f"force_field: {force_field}   # calvados | hps | cocomo | mpipi",
        "",
    ...
    lines.append("")
    lines.append("# Topology entanglement check (runs on final.pdb after MD)")
