...
class Sim:
    ...
    def simulate(self):
        ...
        if (os.path.isfile(fcheck_in)) and (self.restart == 'checkpoint'):
            if not os.path.isfile(f'{self.path}/{self.sysname:s}.xtc'):
                raise Exception(f'Did not find {self.path}/{self.sysname:s}.xtc trajectory to append to!')
            append = True
            print(f'Reading check point file {fcheck_in}')
            print(f'Appending trajectory to {self.path}/{self.sysname:s}.xtc')
            ...
            if os.path.isfile(f'{self.path}/{self.sysname:s}.xtc'): # backup old xtc if not restarting from checkpoint
                now = datetime.now()
                dt_string = now.strftime("%Y%d%m_%Hh%Mm%Ss")
                print(f'Backing up existing {self.path}/{self.sysname:s}.xtc to {self.path}/backup_{self.sysname:s}_{dt_string}.xtc')
                os.system(f'mv {self.path}/{self.sysname:s}.xtc {self.path}/backup_{self.sysname:s}_{dt_string}.xtc')
            print(f'Writing trajectory to new file {self.path}/{self.sysname:s}.xtc')
        ...
        if self.slab_eq:
            ...
            simulation.reporters.append(XTCReporter(f'{self.path}/equilibration_{self.sysname:s}.xtc',self.wfreq,append=append))
        ...
        # run simulation
        simulation.reporters.append(XTCReporter(f'{self.path}/{self.sysname:s}.xtc',self.wfreq,append=append))
